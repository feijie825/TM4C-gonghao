...
BLInternalFlashStartAddrCheck(uint32_t ui32Addr, uint32_t ui32ImgSize)
{
    ...
    //
    ui32FlashSize = ((HWREG(SYSCTL_DC0) & SYSCTL_DC0_FLASHSZ_M) + 1) << 11; //FLASH 大小
    ...
    if((
       ...
       ((ui32Addr + ui32ImgSize) > ui32FlashSize) || ((ui32Addr & 3) != 0)) //代码长度必须为4的倍数 即4字节对齐
    {
        return(0);
    }
    else

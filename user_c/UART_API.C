...
void BOOT_INT_DIS(void)
{
    ...
    IntDisable(INT_UART1);       //UART1         NVIC中断使能  UART1中断
	   IntDisable(INT_UART3);       //UART3         NVIC中断使能  UART3中断
	   IntDisable(INT_UART5);       //UART5         NVIC中断使能  UART5中断
	   IntDisable(INT_UART6);       //UART6         NVIC中断使能  UART6中断
    IntDisable(INT_TIMER0A);     //TIMER0-A      NVIC中断使能  备用高频信号1计数
    IntDisable(INT_TIMER0B);     //TIMER0-B      NVIC中断使能  备用高频信号2计数
    IntDisable(INT_TIMER1A);     //TIMER1-A      NVIC中断使能  备用高频信号3计数
    IntDisable(INT_TIMER1B);     //TIMER1-B      NVIC中断使能  500kHz标准晶振计数
    IntDisable(INT_TIMER2A);     //TIMER2-A      NVIC中断使能  标准表高频计数溢出中断 
    IntDisable(INT_TIMER2B);     //TIMER2-B      NVIC中断使能  标准表无功高频计数溢出中断
	   IntDisable(INT_TIMER3A);     //TIMER3_A      NVIC中断使能  被检表时钟脉冲计数溢出中断
	   IntDisable(INT_TIMER3B);     //TIMER3_B      NVIC中断使能  被检表正向有功脉冲计数溢出中断
    IntDisable(INT_TIMER4A);     //TIMER4-A      NVIC中断使能  被检表正向无功脉冲计数溢出中断
    IntDisable(INT_TIMER4B);     //TIMER4-B      NVIC中断使能  被检表反向有功脉冲计数溢出中断 
    IntDisable(INT_TIMER5A);     //TIMER5-A      NVIC中断使能  被检表反向无功脉冲计数溢出中断 
   	IntDisable(INT_CAN0);        //CAN           NVIC中断使能  CAN 中断
//    IntDisable(INT_WATCHDOG);    //看门狗        NVIC中断使能
    IntDisable(INT_GPIOA);       //GPIOA         NVIC中断使能  GPIOA中断 按键输入
    IntDisable(INT_GPIOM);       //GPIOM         NVIC中断使能  GPIOM中断 光电头脉冲输入 需量脉冲 时段投切 合闸脉冲
    IntDisable(INT_GPION);       //GPION         NVIC中断使能  GPION中断 续流保护继电器状态输入引脚中断
				IntDisable(INT_ADC0SS0);     //ADC0SSS0      NVIC中断使能  ADC0转换中断	
...
void Proc_Com_IBuf(u8 COM)
{
    ...
    if(Com_IHead[COM]<7)                    //长度小于7 认为是干扰
     {
    ...
    else if(CAN_LMSG_TX_TYPE!=COM)          //判断正在发送的是否为COM数据
     return;                                //退出

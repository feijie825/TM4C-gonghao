...
* 设置SSI数据长度
*****************************************************************************/
void SSIDataLen(u32 SSIx,u8 Len)
{
...
void Update_N_Buf(void)
{
    ...
    if(Disp_Choose==DISP_TEST_DATA)        //显示试验数据
     {	        
      if(GZ_FLAG||NZTZ_FLAG)               //是否处于故障和跳闸状态
       return;                             //退出 不再显示其他内容	 
      if(SOLID_CFG.LED_NUM==LED_6)         //是否为6位数码管
       Disp_Buf[0]=(CURRENT_N_ASC[1]&0x0f);//圈数最低位 6位数码管只显示1位圈数 
      else
       {	
